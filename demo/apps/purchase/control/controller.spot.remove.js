	fn.app.purchase.spot.dialog_remove = function() {
		var item_selected = $("#tblSpot").data("selected");
		$.ajax({
			url: "apps/purchase/view/dialog.spot.remove.php",
			...
			success: function(html){
				...
				$("#dialog_remove_spot .btnConfirm").click(function(){
					fn.app.purchase.spot.remove();
				});
			}
		});
	};

	fn.app.purchase.spot.remove = function(id){
		if(typeof id != "undefined"){
			fn.dialog.confirmbox("Confirmation","Are you sure to remove this item?",function(){
				$.post("apps/purchase/xhr/action-remove-spot.php",{item:id},function(response){
			...
			var item_selected = $("#tblSpot").data("selected");
			$.post("apps/purchase/xhr/action-remove-spot.php",{items:item_selected},function(response){
	...
	$(".btn-area").append(fn.ui.button({
		...
		icon_type : "material",
		icon : "delete",
		onclick : "fn.app.purchase.spot.dialog_remove()",
		caption : "Remove"
	}));
