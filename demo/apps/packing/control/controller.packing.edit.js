	fn.app.packing.packing.dialog_edit = function(id) {
		$.ajax({
			url: "apps/packing/view/dialog.packing.edit.php",
			data: {id:id},
			type: "POST",
			dataType: "html",
			success: function(html){
				...
				$("input[name=total_item]").change(function(){
					...
					var iterator = parseInt($("form[name=form_editpacking]").attr("data-iterator"));
					var s = '';
	...
	fn.app.packing.packing.edit = function(){
		$.post("apps/packing/xhr/action-edit-packing.php",$("form[name=form_editpacking]").serialize(),function(response){
			if(response.success){
