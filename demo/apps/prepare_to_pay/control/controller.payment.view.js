...
$("#tblPayment").DataTable({
	...
	"processing": true,
	"serverSide": true,
	"ajax": "apps/prepare_to_pay/store/store-payment.php",	
	"aoColumns": [
	...
	"createdRow": function ( row, data, index ) {
		...
		s = '';
		if(data.status =="1"){
			$("td", row).eq(10).html('<span class="badge badge-dark">เตรียมแพ็คแล้ว</span>');
		}else{
			$("td", row).eq(10).html('<span class="badge badge-warning">ยังไม่ได้เตรียม</span>');
		}
		s += fn.ui.button("btn btn-xs btn-danger","far fa-dollar-sign","fn.app.prepare_to_pay.payment.dialog_edit("+data[0]+")");
		$("td", row).eq(11).html(s);
	}
});
