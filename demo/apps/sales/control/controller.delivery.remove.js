	fn.app.sales.delivery.dialog_remove = function() {
		var item_selected = $("#tblDelivery").data("selected");
		$.ajax({
			url: "apps/sales/view/dialog.delivery.remove.php",
			...
			success: function(html){
				...
				$("#dialog_remove_delivery .btnConfirm").click(function(){
					fn.app.sales.delivery.remove();
				});
			}
		});
	};

	fn.app.sales.delivery.remove = function(id){
		if(typeof id != "undefined"){
			fn.dialog.confirmbox("Confirmation","Are you sure to remove this item?",function(){
				$.post("apps/sales/xhr/action-remove-delivery.php",{item:id},function(response){
			...
			var item_selected = $("#tblDelivery").data("selected");
			$.post("apps/sales/xhr/action-remove-delivery.php",{items:item_selected},function(response){
	...
	$(".btn-area").append(fn.ui.button({
		...
		icon_type : "material",
		icon : "delete",
		onclick : "fn.app.sales.delivery.dialog_remove()",
		caption : "Remove"
	}));
