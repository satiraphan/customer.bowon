<table class="datatable table table-striped table-sm table-bordered dt-responsive nowrap">
	...
	<?php
		...
		$sql = "SELECT 
			...
			WHERE
				DATE(bs_deliveries.delivery_date) LIKE '".$date."'
		...
		while($order = $dbc->Fetch($rst)){
			...
			/*
				...
				*/
				...
				echo '<td>'.number_format($order['amount'],4).'</td>';
				echo '<td>'.number_format($order['price'],4).'</td>';
				echo '<td>'.number_format($order['net'],4).'</td>';
